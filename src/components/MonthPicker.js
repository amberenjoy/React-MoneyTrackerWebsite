    ...
    render() {
        ...
        return (
            <div className='dropdown month-picker-component' ref={(ref) => { this.node = ref }}>
                {/* <h4>Pick a Month</h4> */}
                <button className='btn btn-lg btn-secondary dropdown-toggle px-5 ' onClick={this.toggleDropdown} >
                    {`${year}.${padLeft(month)}.`}
                </button>
                {
                    isOpen &&
                        ...
                        <div className='row'>
                            <div className='col border-right years-range'>
                                {yearRange.map((yearNumber, index) =>
                                    <a href='#' key={index} className={(yearNumber === selectedYear) ? 'dropdown-item active' : 'dropdown-item'}
